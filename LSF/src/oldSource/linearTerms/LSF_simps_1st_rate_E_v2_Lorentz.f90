...
function Aj(t) result (A)
use capcs
    real(kind=dp) :: t, wt, omega, nj
    complex(kind=dp) :: A, A1, A2, eiwt
    complex(kind=dp) :: tmp1 ! n * e ^ (-iwt)
    complex(kind=dp) :: tmp2 ! (n + 1) * e ^ (iwt)
    integer :: ifreq
    A = 0.0_dp
    do ifreq=1, nfreq
        omega = ipfreq(ifreq)
        nj = 1 / (exp(hbar * omega * beta) - 1)
        wt = omega * t
        eiwt = exp(cmplx(0.0, wt,dp))
        tmp1 = nj * conjg(eiwt)
        tmp2 = (1 + nj) * eiwt
        A1 = tmp1 + tmp2
        A2 = Sj(ifreq) * (1 + tmp1 - tmp2) ** 2
        A = A + 0.25_dp * Mj(ifreq) * (2 * hbar / omega) * (A1 + A2)
    end do
end function
...
DO i=1, nE
   ...
   S(i) = S(i) + Real(Aj(t1)*exp(G0_t(t1)+cmplx(0.0,t1*Eif/hbar,dp)-gamma0*t1)) ! Lorentz
   S(i) = S(i) - Real(Aj(t2)*exp(G0_t(t2)+cmplx(0.0,t2*Eif/hbar,dp)-gamma0*t2)) ! Lorentz
   S(i) = S(i)*2.0d0/3.0d0*dstep
   S(i) = S(i)/hbar/hbar
   write(*,'(f10.5,f7.1,i5,ES25.14E3)') EeV(i), temperature, nn, S(i)
END DO
end program

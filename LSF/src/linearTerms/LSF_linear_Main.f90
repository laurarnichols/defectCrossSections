program lineShapeFunction
  ...
  call MPI_BCAST(modes   ,  1, MPI_INTEGER,root,MPI_COMM_WORLD,ierr)
  ...
  !
  if ( myid /= root ) then
    allocate( phonF(nModes), x(nModes), Sj(nModes), coth(nModes), wby2kT(nModes), s2L(nModes) )
...
!    allocate( Vfis(-nEnergies:nEnergies) )
  ...
  call MPI_BCAST( s2L, size(s2L), MPI_INTEGER,root,MPI_COMM_WORLD,ierr)
!  call MPI_BCAST( Vfis, size(Vfis), MPI_DOUBLE_PRECISION,root,MPI_COMM_WORLD,ierr)
  ...
  if ( myid == root ) then
    ...
    if ( ( minimumNumberOfPhonons < 2 ) .and. ( maximumNumberOfPhonons > 0 ) .and. ( modes > 0 ) ) then
      !
      ! One phonon
      !
...
!twoPi*abCM**2*volume*Vfis(iE)*lsfVsE(iE)/vg
    ...
    !
    if ( ( minimumNumberOfPhonons < 3 ) .and. ( maximumNumberOfPhonons > 1 ) .and. ( modes > 1 ) ) then
      !
      ! Two phonons
      !
      lsfVsEbyBands(:) = 0.0_dp
...
!twoPi*abCM**2*volume*Vfis(iE)*lsfVsE(iE)/vg
  ...
  if ( ( minimumNumberOfPhonons < 4 ) .and. ( maximumNumberOfPhonons > 2 ) .and. ( modes > 2 ) ) then
    !
    lsfVsEbyBands(:) = 0.0_dp
    iEbinsByBands(:) = 0
    !
...
!twoPi*abCM**2*volume*Vfis(iE)*lsfVsE(iE)/vg
  ...
  if ( ( minimumNumberOfPhonons < 5 ) .and. ( maximumNumberOfPhonons > 3 ) .and. ( modes > 3 ) ) then
    !
    lsfVsEbyBands(:) = 0.0_dp
    iEbinsByBands(:) = 0
    !
...
!twoPi*abCM**2*volume*Vfis(iE)*lsfVsE(iE)/vg
  ...
  !
  if ( ( maximumNumberOfPhonons > 4 ) .and. ( modes > 4 ) ) then
    ...
    do m = minimumNumberOfPhonons, maximumNumberOfPhonons
      ...
      do l = 4, m
        ...
        if ( myid == root ) then
          ...
          !
          do iMode = 2, l
            weight = weight*(modes - iMode + 1)/iMode
          enddo
          !
